<section class="logo_box clearfix">
    <div class="fl">
        <a href="/">
            <img class="logo_img" src="/static/img/O4422o9131EZHHCGO1mCmq46Ot96T4.png" alt="logo">
        </a>
    </div>
    <div class="sosuo_box fl">
        <form action="/search" method="post" role="form">
            <input class="btn_com btn_sosuo" id="text" type="text" placeholder="请输入影视、电视剧关键词、支持拼音" name="key" value="">
        </form>
    </div>
    <div class="fr">
        <a class="tanchu" href="javascript:void(0)">
            <em class="jilu"></em>
        </a>
    </div>
</section>

<section class="guanyin_box guanyin_box2">
    <div class="meiyou_box">
        <p class="zhanshi_p">最近十条观影记录</p>
        <div class="jilu_box">
            <ul class="clearfix">

            </ul>
        </div>
    </div>
    <div class="fanhui_box2 fanhui_box3">
        <p class="fanhui2_p" style="padding-bottom: 15px">
            <a class="fanhui_dianji" href="javascript:void(0)" id="clean" style="color: #fff">清空记录</a>
        </p>
        <a class="fanhui_dianji" href="javascript:void(0)">
            <em class="close_2"></em>
        </a>
    </div>
</section>
